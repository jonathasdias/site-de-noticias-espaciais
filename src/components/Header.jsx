...
export default function Header() {
    ...
    return(
        <header className='flex justify-between items-center px-4 py-2 bg-gray-300'>
            ...
            <Link to='/' className='block font-black text-7xl'>Space News</Link>

            <button onClick={openMenu} className='text-4xl block lg:hidden' aria-label='Menu' aria-pressed={isOpen}><CgMenuGridR aria-hidden='true'/></button>
            ...
            <section aria-label='Menu' tabIndex='0' className={`fixed top-0 ${isOpen ? 'right-0' : 'right-[-15rem]'} ${isHidden ? 'hidden' : 'block'} z-10 lg:hidden bg-gray-300 w-60 h-full transition-all ease-out duration-500`}>
                ...
                <nav className='mt-12'>
                    <NavLink to="/" className='block text-center px-6 py-2 hover:bg-gray-200 aria-[current=page]:bg-gray-500 rounded-md text-xl'>Home</NavLink>
                    <NavLink to="/articles" className='block text-center px-6 py-2 hover:bg-gray-200 aria-[current=page]:bg-gray-500 rounded-md text-xl'>Articles</NavLink>
                    <NavLink to="/blogs" className='block text-center px-6 py-2 hover:bg-gray-200 aria-[current=page]:bg-gray-500 rounded-md text-xl'>Blogs</NavLink>
                    <NavLink to="/reports" className='block text-center px-6 py-2 hover:bg-gray-200 aria-[current=page]:bg-gray-500 rounded-md text-xl'>Reports</NavLink>
                </nav>
            </section>
